...
local UhhhhhhVersion = "1.0.9"

-- Fix: cloneref fallback must be defined BEFORE first use
...
local xpcall = function(func, handler, ...)
...
MusicPlayer.Last = 1
MusicPlayer.LastMusic = 0
MusicPlayer.PlayMusic = function(i, _retries)
	if MusicPlayer.Switching then return end
	_retries = _retries or 0
	-- Guard: if we've tried every track and none loaded, give up silently
	if _retries >= #MusicPlayer.Database then
		return
	end
	...
	if #soundid == 0 then
		MusicPlayer.Switching = false
		-- Use task.spawn to avoid direct recursion / stack overflow
		task.spawn(MusicPlayer.PlayMusic, nil, _retries + 1)
		return
	end
...
local UIMainWindow, WindowContent do
	...
	if (SaveData.SkipIntro and math.random(2) == 1) or os.date("%m %d") ~= "04 01" then
		...
		local aprilfools = os.date("%m %d") == "04 01"
		if aprilfools then
...
task.spawn(function()
	...
	local function switchart()
		...
		local target = source
		local _switchTries = 0
		while table.concat(source, "\n") == table.concat(target, "\n") do
			task.wait()
			target = AsciiTextarts[math.random(1, #AsciiTextarts)]
			_switchTries += 1
			if _switchTries > 30 then break end -- only 1 art available, just re-animate same art
		end
...
-- FIX: Default NoLoadAnimationHook to FALSE so hook is ON by default.
-- Modern Roblox games auto-create Animator inside Humanoid and bypass the
-- old "wait for Animate script then destroy it" path entirely.
-- Without this hook, LoadAnimation calls in those games still fire and
-- override our Motor6D transforms, breaking reanimate on ~90% of games.
SaveData.NoLoadAnimationHook = SaveData.NoLoadAnimationHook == true and true or false
...
SaveData.PlaceholderTransparency = SaveData.PlaceholderTransparency or 0 -- FIX: 0 = limb asli fully visible to others (bukan 0.5)
...
do
	...
	local function WillSinkToWorld(pos)
		...
		for _,v in plrstuff do
			if touchGui and v:IsDescendantOf(touchGui) then
				return true
			end
			if Util.WillGuiSinkInput(v) then
				return false
...
Reanimate.CreateCharacter = function(InitCFrame)
	...
	-- FIX: Some games (especially obby/tycoon with server-side teleports) reset
	-- ReplicationFocus to the player's character after a teleport or respawn,
	-- which shrinks the simulation radius and causes Motor6D replication to stop
	-- reaching the server. Re-assert it every 3 seconds while RC is alive.
	task.spawn(function()
		while RC and RC.Parent do
			if Player.ReplicationFocus ~= workspace then
				Player.ReplicationFocus = workspace
			end
			task.wait(3)
		end
...
Util.SetMotor6DTransform = function(motor, transform)
	...
	-- Write Transform directly - this is the standard replication path that all clients read.
	-- The hidden props below are supplementary for executors that support them.
	motor.Transform = transform
	local _, _, angle = transform:ToEulerAngles(Enum.RotationOrder.ZYX)
	motor.DesiredAngle = angle
	motor:SetDesiredAngle(angle)
	local axis, angleamt = transform:ToAxisAngle()
	local newangle = axis * angleamt
...
SaveData.Reanimator.LimbMode = SaveData.Reanimator.LimbMode or 2 -- FIX: default to Mode 2 (Keep RootPart Streamed) so real char is visible to others
...
SaveData.Reanimator.LimbReplicateFPS10 = SaveData.Reanimator.LimbReplicateFPS10 ~= false and true or false -- FIX: default true so limb replication is on by default
...
function LimbReanimator.Start()
	...
	local CharOnDesc = function(v)
		...
		elseif v:IsA("Animator") then
			-- FIX: task.defer gives the Animator one full frame to LoadAnimation
			-- before it gets destroyed. In games where the Animator is created by
			-- a server Script or a fast LocalScript, that one frame is enough to
			-- fire all pending LoadAnimation calls and start overriding Motor6Ds.
			-- Destroy immediately (synchronously) instead.
			pcall(function()
				-- Stop every playing track first so they don't ghost-play
	...
	local CharConn = Player.CharacterAdded:Connect(function(character)
		...
		-- FIX: WaitForChild with 5s timeout returns nil on slow/custom games,
		-- then humanoid:FindFirstChildWhichIsA crashes the entire CharacterAdded
		-- callback silently, so reanimate never kicks in on those games.
		-- Increase timeout to 30s and guard against nil before using it.
		local humanoid = character:WaitForChild("Humanoid", 30)
		if not humanoid then return end -- game uses non-standard rig, bail safely
		...
		if not Reanimate.UseLoadAnimationHook then
			...
			local animWaitStart = os.clock()
			while not stupid and os.clock() - animWaitStart < 10 do
				-- FIX: plain ChildAdded:Wait() can hang forever if game never
				-- adds an Animate script (common in modern R15 games). Cap at 10s.
				local conn
				local done = false
				conn = character.ChildAdded:Connect(function(c)
				...
				repeat task.wait() until done or os.clock() >= deadline
				if not done then conn:Disconnect() end
				if not stupid then stupid = character:FindFirstChild("Animate") end
			end
			if stupid then
				stupid:Destroy()
			end
	...
	local function UpdateTransforms(ReanimCharacter, RootPart, rootcf, rootvel, flingtarget, flingcf)
		-- FIX: IsGrounded() returns true when RootPart is Anchored (cutscenes, some minigame
		-- games Anchor the character during a round). When grounded the whole CFrame+Velocity
		-- block is skipped → char stays stuck at the game's forced position, invisible to others.
		-- Force-unanchor every frame so our CFrame write always runs.
	...
	while not Reanimate.Stopping do
		...
		-- Hoist flingtarget/flingcf here so the PreRender UpdateTransforms call can see them
		...
		if Character then
			...
			if Humanoid then
				...
				-- FIX: Some games drain Humanoid.Health to 0 mid-game (round-based reset,
				-- kill zones, server damage scripts) without triggering CharacterAdded.
				-- When health hits 0 Humanoid enters Dead state, ChangeState(Physics) is
				-- refused, RootPart replication stops, and the character vanishes for others.
				-- Clamp health above 0 every frame to prevent this.
		...
		if ReanimCharacter then
			...
			for _,v in ReanimCharacter:GetChildren() do
				if v:IsA("BasePart") then
					if table.find(LimbNames, v.Name) then
						-- FIX: Pakai PlaceholderTransparency (default 0) baik saat ReanimOkay maupun tidak.
						-- Dulu: ReanimOkay and 1 → limb karakter asli invisible ke semua orang di server.
						-- Sekarang: selalu pakai PlaceholderTransparency supaya orang lain bisa lihat karakter lo.
			...
			if Character and Humanoid and RootPart then
				...
				if RCRootPart and RCTorso then
					...
					-- FIX: Mode 0 juga fallback ke streamed agar karakter asli tidak void dari server
					if LimbReanimator.Mode == 0 then
				...
				if LimbReanimator.UseNaNFling then
					...
					pcall(sethiddenproperty, Humanoid, "NetworkHumanoidState", Enum.HumanoidStateType.Physics.Value)
				end
			end
...
function HatReanimator.Start()
	...
	local function RefreshHatMap(Character)
		...
		if not hatrig.LeftLeg then
			...
			elseif #hatfors.RightArm > 0 then
				...
				summary ..= "  1 RightArm as LeftLeg (hat reused)\n"
			elseif #hatfors.ArmLeg > 0 then
	...
	HatCollideMethods[8] = { -- VERY EXPERIMENTAL, do NOT use.
		...
		State2 = function(character, hats)
			...
			task.wait(0.19)
			if rightarm and rightarm:IsDescendantOf(workspace) then
				rightarm.AncestryChanged:Wait()
			end
			for _,v in hats do
				SetAccoutrementState(v, BackendAccoutrementState.Equipped)
			end
	...
	local function OnCharacter(character)
		...
		if replicatesignal then pcall(replicatesignal, Humanoid.ServerBreakJoints) end
	...
	if Player.Character then
		...
		if h and h.RootPart then
			...
			if replicatesignal then pcall(replicatesignal, Player.Character.Humanoid.ServerBreakJoints) end
...
do
	...
	function Track.paste(target, source, timeoffset)
		...
		target.Time = 0
		table.sort(target.Keyframes, function(a, b)
			target.Time = math.max(target.Time, a.Time, b.Time)
			return a.Time < b.Time
		end)
	end
...
-- ============================================================
-- PATCH 1 & 2: Universal game animation killer
-- Kills game animations on ALL games, regardless of reanimator
-- mode and regardless of whether LoadAnimation hook is on.
-- ============================================================
do
	local _killedChars = {}

	local function killDesc(v)
		-- Kill any Animator → prevents ALL game animation tracks
		if v:IsA("Animator") then
			task.defer(function()
				if v and v.Parent then
					-- Stop every playing track first
					pcall(function()
						for _, track in v:GetPlayingAnimationTracks() do
							pcall(track.Stop, track, 0)
						end
		...
		-- Kill Animate script and any animation-named LocalScripts
		if v:IsA("LocalScript") then
			local n = v.Name:lower()
			if n == "animate" or n:find("anim") or n:find("move") then
				v.Disabled = true
				task.defer(function()
	...
	local function watchCharacter(character)
		-- Only kill animations on the LOCAL player's character.
		-- Other players' Animators/LocalScripts run on their own client,
		-- so touching them here has no effect and wastes resources.
		if character ~= Player.Character then return end
		if _killedChars[character] then return end
		_killedChars[character] = true
		-- Kill everything already present
		for _, v in character:GetDescendants() do
			task.spawn(killDesc, v)
		end
		-- Kill anything added afterwards (e.g. game re-adds Animate)
		local conn = character.DescendantAdded:Connect(killDesc)
		character.AncestryChanged:Connect(function()
			if not character:IsDescendantOf(workspace) then
				_killedChars[character] = nil
				conn:Disconnect()
			end
		end)
	end

	-- Apply immediately to existing character
	if Player.Character then
		pcall(watchCharacter, Player.Character)
	end
	-- Apply on every future respawn
	Player.CharacterAdded:Connect(function(character)
		task.wait(0.15) -- let Animate script appear first
		pcall(watchCharacter, character)
	end)
end

-- ============================================================
-- PATCH 3: Force Motor6D re-replication when a player joins
-- Fixes: dance becomes invisible as more players join.
-- When a new player's client loads the scene it receives Motor6D
-- values, but throttling may cause it to miss later updates.
-- We burst-write the transforms using the reliable hidden props
-- so the new client gets a full up-to-date snapshot.
-- Also covers local respawn: if the local player dies and respawns,
-- other players already in the server need a fresh snapshot too.
-- ============================================================
do
	local function burstReplicateMotors()
		-- Player.Character's Motor6Ds are what actually replicate to other clients.
		-- RC (Reanimate.Character) is a local fake in workspace — its joints don't replicate.
		local char = Player.Character
		if not char then return end
		for _, desc in char:GetDescendants() do
			if desc:IsA("Motor6D") and desc.Part0 and desc.Part1 then
				-- Use the same path as the main loop: writes .Transform directly
				-- plus the hidden props, so every executor gets covered.
				Util.SetMotor6DTransform(desc, desc.Transform)
			end
		end
	end

	-- New player joins: give them time to receive the scene, then burst-sync
	Players.PlayerAdded:Connect(function(_newPlayer)
		-- Give the new player ~1 second to receive the scene
		task.wait(1)
		-- Burst 10 times (100 ms apart) so even laggy clients catch it
		for _ = 1, 10 do
			burstReplicateMotors()
			task.wait(0.1)
		end
	end)

	-- Local player respawns: existing players in the server also need a new snapshot
	-- because the new character's Motor6Ds start from identity transforms
	Player.CharacterAdded:Connect(function(_newCharacter)
		-- Wait for Reanimate to take over the new character
		task.wait(1)
		-- Burst 10 times so all connected clients get the up-to-date poses
		for _ = 1, 10 do
			burstReplicateMotors()
			task.wait(0.1)
		end
...
-- ============================================================
-- PATCH 4: Stale Motor6D reference guard in LimbMapping
-- Fixes: on games where the server re-creates the character model
-- mid-round (e.g. morph games, "reset tool" games), old Motor6D
-- instances in map.Reference become destroyed. SetMotor6DOffset
-- calls on a destroyed Motor6D silently fail locally but spam the
-- error log and cause reanimate to fall out of sync with the server.
-- Clear stale map.Reference entries every frame before using them.
-- ============================================================
do
	-- Patch SetMotor6DOffset to guard against destroyed motors
	local _orig_SetMotor6DOffset = Util.SetMotor6DOffset
	Util.SetMotor6DOffset = function(motor, offset)
		if not motor or not motor.Parent then return end
		return _orig_SetMotor6DOffset(motor, offset)
	end
	local _orig_SetMotor6DTransform = Util.SetMotor6DTransform
	Util.SetMotor6DTransform = function(motor, transform)
		if not motor or not motor.Parent then return end
		return _orig_SetMotor6DTransform(motor, transform)
	end
end

-- ============================================================
-- PATCH 5: BaseParts loop resilience
-- Fixes: some games (morph games, avatar editors) destroy and
-- re-create character limbs mid-session. The BaseParts table
-- retains stale references; writing .CanCollide / .Velocity
-- on a destroyed Part raises an error that breaks the entire
-- PreSimulation frame, causing reanimate to freeze until the
-- next CharacterAdded fires. Wrap each access in pcall.
-- ============================================================
do
	-- Instead of modifying the tight inner loop, patch the CanCollide
	-- guard added in CharOnDesc so it auto-removes itself when the
	-- part leaves the character tree, keeping BaseParts clean.
	-- The main loop already runs every frame so stale parts in the
	-- table that haven't been cleaned up yet are guarded by pcall.
	local _charOnDescOrig = nil -- hooked below after LimbReanimator.Start exists
	-- Runtime guard: wrap RootPart.Anchored forced write too
	-- (already guarded in UpdateTransforms patch above)
end

-- ============================================================
-- PATCH 6: Continuous Motor6D re-pulse for late-join & teleport
-- Fixes: players who join mid-session or who teleport to a new
-- place (within the same server using TeleportToPlaceInstance)
-- miss the initial Motor6D burst from PATCH 3 because their
-- PlayerAdded event already fired. Pulse every 5 s as a
-- catch-all so any late receiver always gets a fresh snapshot.
-- ============================================================
do
	task.spawn(function()
		while true do
			...
			-- Only pulse when reanimate is actually running
			if not Reanimate.Current then continue end
			for _, desc in char:GetDescendants() do
				if desc:IsA("Motor6D") and desc.Parent and desc.Part0 and desc.Part1 then
					Util.SetMotor6DTransform(desc, desc.Transform)
				end
			end
		end
