...
-- ============================================================
-- PATCH 4: Continuous RC -> Player.Character Motor6D sync
-- Fixes: dance/pose invisible to others on ALL games.
-- Emper Reanimate writes transforms to RC (fake local character),
-- but server replication only reads Player.Character's Motor6Ds.
-- This patch mirrors RC's Motor6D transforms onto Player.Character
-- every frame so other players always see your current pose/dance.
-- Works on all Roblox games regardless of animator setup.
-- ============================================================
do
	-- Build a name->Motor6D map for Player.Character for fast lookup
	...
	local function rebuildCache(char)
		...
		for _, desc in char:GetDescendants() do
			if desc:IsA("Motor6D") then
				-- Use Part0.Name+Part1.Name as key to handle duplicate names
	...
	-- Rebuild cache on each respawn
	Player.CharacterAdded:Connect(function(char)
		task.wait(0.5) -- wait for Motor6Ds to be parented
		rebuildCache(char)
	end)
	if Player.Character then
		task.spawn(function()
	...
	RunService.Heartbeat:Connect(function()
		local now = os.clock()
		-- Sync at 45 Hz — smooth enough to see, not heavy on perf
		...
		-- Mirror every Motor6D from RC to Player.Character
		for _, rcMotor in RC:GetDescendants() do
			if rcMotor:IsA("Motor6D") and rcMotor.Part0 and rcMotor.Part1 then
				...
				-- Try specific key first, then name-only fallback
