...
Util.SetMotor6DTransform = function(motor, transform)
	...
	-- Primary replication path: .Transform is what Roblox replicates
	-- from the network owner to all other clients.
	motor.Transform = transform

	-- Force CurrentAngle to match — some executors/games use this
	-- hidden prop as the actual replicated joint angle. Without it,
	-- other players may see the default T-pose regardless of Transform.
	local axis, angleamt = transform:ToAxisAngle()
	-- CurrentAngle is a single float (radians around the hinge axis).
	-- For Motor6D used as a general 6DOF joint we write the magnitude.
	local flatAngle = (angleamt ~= angleamt or angleamt == 0) and 0 or angleamt
	pcall(sethiddenproperty, motor, "CurrentAngle", flatAngle)
	motor.DesiredAngle = flatAngle
	pcall(motor.SetDesiredAngle, motor, flatAngle)

	-- ReplicateCurrentAngle6D: Vector3 of axis*angle — correct format
	local angle6D = (angleamt ~= angleamt) and Vector3.zero or axis * angleamt
	pcall(sethiddenproperty, motor, "ReplicateCurrentAngle6D", angle6D)

	-- ReplicateCurrentOffset6D: the TRANSLATION part of the transform,
	-- i.e. how far Part1 is offset from its rest position along Part0.
	-- This is transform.Position, not a world position.
	local off6D = transform.Position
	pcall(sethiddenproperty, motor, "ReplicateCurrentOffset6D", off6D)
end
		...
		if not RootPart:IsGrounded() then
			if flingtarget then
				if LimbReanimator.UseNaNFling then
					...
					RootPart.AssemblyLinearVelocity, RootPart.AssemblyAngularVelocity = Vector3.new(0, -16384, 0), Vector3.one * 16384
				end
				...
				RootPart.AssemblyLinearVelocity, RootPart.AssemblyAngularVelocity = rootvel, Vector3.zero
		...
		if ReanimCharacter then
			...
			if Character and Humanoid and RootPart then
				...
				if LimbReanimator.UseNaNFling then
					...
					-- PATCH: "Physics" state causes the server to skip relaying
					-- joint transform data to other clients (server treats the
					-- character as fully client-simulated and doesn't broadcast
					-- its Motor6D changes). Use "Running" instead — the server
					-- still accepts and relays joint updates while the character
					-- appears normally animated to other players.
					pcall(sethiddenproperty, Humanoid, "NetworkHumanoidState", Enum.HumanoidStateType.Running.Value)
				end
			end
...
-- ============================================================
-- PATCH 3 & 4 (UPGRADED): Universal R6 + R15 Motor6D visibility sync
-- Fixes: dance/pose invisible to other players on ALL games,
-- for both R6 and R15 rigs.
--
-- Root cause: RC (Reanimate.Character) is a LOCAL fake R6 model
-- in workspace. Its joints do NOT replicate to other clients.
-- Only Player.Character's Motor6Ds replicate. This patch:
--   1. Detects whether the real character is R6 or R15.
--   2. If R6: mirrors RC joints directly (name-matched).
--   3. If R15: maps the 6 R6 RC joints to the 15 R15 joints
--      using anatomical decomposition so other players see
--      the correct pose for every limb segment.
--   4. Burst-replicates on player join and on local respawn.
-- ============================================================
do
	-- -------------------------------------------------------
	-- R15 joint name list (all Motor6Ds in a standard R15 rig)
	-- -------------------------------------------------------
	...
	-- -------------------------------------------------------
	-- Build Motor6D lookup cache for Player.Character
	-- Keys: "MotorName|Part0Name|Part1Name" and "MotorName"
	-- -------------------------------------------------------
	local charMotorCache = {}
	local lastCachedChar = nil
	local lastDetectedRig = "unknown"

	local function rebuildCache(char)
		table.clear(charMotorCache)
		lastCachedChar = char
		lastDetectedRig = "unknown"
		if not char then return end
		lastDetectedRig = detectRigType(char)
		for _, desc in char:GetDescendants() do
			if desc:IsA("Motor6D") then
				local p0 = desc.Part0 and desc.Part0.Name or ""
				local p1 = desc.Part1 and desc.Part1.Name or ""
				charMotorCache[desc.Name .. "|" .. p0 .. "|" .. p1] = desc
				if not charMotorCache[desc.Name] then
					charMotorCache[desc.Name] = desc
				end
			end
		end
	end
	...
	-- -------------------------------------------------------
	-- R6 RC joint name -> R15 Player.Character joint name(s)
	-- RC always has 6 standard R6 joints. We map each one to
	-- the R15 joints that cover the same anatomical region.
	-- The transform is split evenly across child joints when
	-- there are multiple (e.g. UpperArm + LowerArm + Hand).
	-- -------------------------------------------------------
	-- Each entry: { rcJointName, { r15JointName, ... }, splitMode }
	-- splitMode "full"  = apply same transform to every listed joint
	-- splitMode "root"  = only apply to first joint, rest identity
	-- splitMode "split" = divide rotation evenly (more natural look)
	local R6_TO_R15_MAP = {
		-- RootJoint -> Root (HumanoidRootPart -> LowerTorso)
		{ rc = "RootJoint",      r15 = {"Root"},                                  mode = "full" },
		-- Neck -> Waist (LowerTorso->UpperTorso) + Neck (UpperTorso->Head)
		{ rc = "Neck",           r15 = {"Waist", "Neck"},                         mode = "split" },
		-- Left Shoulder -> LeftShoulder + LeftElbow + LeftWrist
		{ rc = "Left Shoulder",  r15 = {"LeftShoulder", "LeftElbow", "LeftWrist"}, mode = "split" },
		-- Right Shoulder -> RightShoulder + RightElbow + RightWrist
		{ rc = "Right Shoulder", r15 = {"RightShoulder", "RightElbow", "RightWrist"}, mode = "split" },
		-- Left Hip -> LeftHip + LeftKnee + LeftAnkle
		{ rc = "Left Hip",       r15 = {"LeftHip", "LeftKnee", "LeftAnkle"},      mode = "split" },
		-- Right Hip -> RightHip + RightKnee + RightAnkle
		{ rc = "Right Hip",      r15 = {"RightHip", "RightKnee", "RightAnkle"},   mode = "split" },
	}

	-- Helper: split a CFrame rotation evenly across N joints
	-- Returns a table of N CFrames. Position only goes into first joint.
	local function splitTransform(transform, n)
		if n <= 1 then return {transform} end
		-- Extract axis-angle from rotation, divide angle by n
		local axis, angle = transform.Rotation:ToAxisAngle()
		local pos = transform.Position
		local results = {}
		for i = 1, n do
			local rot = CFrame.fromAxisAngle(axis, angle / n)
			if i == 1 then
				results[i] = rot + pos
			else
				results[i] = rot
	...
	-- -------------------------------------------------------
	-- Apply RC Motor6D transforms to Player.Character
	-- Handles both R6->R6 (direct) and R6->R15 (mapped) cases
	-- -------------------------------------------------------
	local function syncMotors(RC, char)
		local rigType = lastDetectedRig

		if rigType == "R6" or rigType == "unknown" then
			-- Direct name match (original behaviour, kept intact)
			for _, rcMotor in RC:GetDescendants() do
				if rcMotor:IsA("Motor6D") and rcMotor.Part0 and rcMotor.Part1 then
					local p0 = rcMotor.Part0.Name
					local p1 = rcMotor.Part1.Name
					local charMotor = charMotorCache[rcMotor.Name .. "|" .. p0 .. "|" .. p1]
						or charMotorCache[rcMotor.Name]
					if charMotor then
						Util.SetMotor6DTransform(charMotor, rcMotor.Transform)
					end
				end
			end

		elseif rigType == "R15" then
			-- Map each R6 RC joint to one or more R15 joints
			for _, mapping in R6_TO_R15_MAP do
				-- Find the RC motor by name
				local rcMotor = nil
				for _, desc in RC:GetDescendants() do
					if desc:IsA("Motor6D") and desc.Name == mapping.rc then
						rcMotor = desc
						break
					end
				end
				if rcMotor then
					local transform = rcMotor.Transform
					local r15Names = mapping.r15
					local mode = mapping.mode

					if mode == "full" then
						-- Same transform to all target joints
						for _, r15Name in r15Names do
							local charMotor = charMotorCache[r15Name]
							if charMotor then
								Util.SetMotor6DTransform(charMotor, transform)
							end
						end
					elseif mode == "root" then
						-- Only first joint gets the transform
						local charMotor = charMotorCache[r15Names[1]]
						if charMotor then
							Util.SetMotor6DTransform(charMotor, transform)
						end
						-- Rest get identity
						for i = 2, #r15Names do
							local cm = charMotorCache[r15Names[i]]
							if cm then
								Util.SetMotor6DTransform(cm, CFrame.identity)
							end
						end
					elseif mode == "split" then
						-- Divide rotation evenly across all target joints
						local splits = splitTransform(transform, #r15Names)
						for i, r15Name in ipairs(r15Names) do
							local charMotor = charMotorCache[r15Name]
							if charMotor then
								Util.SetMotor6DTransform(charMotor, splits[i])
							end
						end
					end
				end
	...
	-- -------------------------------------------------------
	-- Burst-replicate: touch every Motor6D on the real char
	-- so the server snapshot is fresh for newly joined players
	-- -------------------------------------------------------
	...
	-- Rebuild cache on respawn (rig type may change between spawns)
	Player.CharacterAdded:Connect(function(char)
		task.wait(0.5) -- wait for Motor6Ds to be added
		rebuildCache(char)
		-- Burst so existing players get a fresh snapshot
		task.wait(0.5)
		for _ = 1, 10 do
	...
	-- New player joins: give them time, then burst current pose
	...
	-- -------------------------------------------------------
	-- Main sync loop: 45 Hz, mirrors RC -> Player.Character
	-- -------------------------------------------------------
	local lastSyncTime = 0
	RunService.Heartbeat:Connect(function()
		local now = os.clock()
		if now - lastSyncTime < 1 / 45 then return end
		lastSyncTime = now

		local RC = Reanimate.Character
		local char = Player.Character
		if not RC or not char then return end

		-- Rebuild cache if character instance changed
		if char ~= lastCachedChar then
			rebuildCache(char)
		end
