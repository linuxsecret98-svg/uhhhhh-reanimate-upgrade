...
do
	...
	if not ismissing(isfile) then
		...
		-- NOTE: cabang ini jalan kalau isfile tidak ada sama sekali.
		-- Seharusnya tidak pernah sampai sini karena ismissing(isfile) sudah dicek
		-- di blok validasi di atas dan akan diefatal duluan.
		-- Tapi kalau refactor nanti, jangan hapus guard ini.
		diefatal("T-this one shouldn't happen!")
	end
end

-- BUG FIX: pcall/xpcall asli harus disimpan dulu sebelum di-shadow,
-- karena kalau langsung shadow diri sendiri → infinite recursion / stack overflow.
local _builtin_pcall = pcall
local _builtin_xpcall = xpcall
local pcall = function(func, ...)
	return _builtin_pcall(function(...)
		return func(...)
	end, ...)
end
local xpcall = function(func, handler, ...)
	return _builtin_xpcall(function(...)
		return func(...)
	end, handler, ...)
end
...
if not game:IsLoaded() then
	...
	game.Loaded:Wait()
	-- BUG FIX: kondisi asli "> 1/10" terbalik — task.wait() return delta time,
	-- kalau delta > 0.1 (koneksi lambat) loop langsung exit tanpa nunggu,
	-- kalau delta <= 0.1 malah infinite loop. Fix: tunggu 3 detik flat setelah loaded.
	task.wait(3)
	notLoaded:Destroy()
end
...
Util.MakeTriforce = function(tris, color, dur)
	...
	pivot.AnchorPoint = Vector2.new(0.5, 0.5)
	pivot.Position = UDim2.new(0.5, 0, 0.5, 0)
	-- BUG FIX: Size tidak di-set → default 0x0 → triforce invisible sampai caller set Size.
	-- Set ke 0 dulu (caller tetap wajib set Size dari luar), tapi sekarang eksplisit.
...
if SaveData.SkipIntro then
	...
	UISound.Music:Stop()
	task.wait()
	-- BUG FIX: TimePosition harus di-set SEBELUM Play(), bukan sesudahnya,
	-- supaya audio engine tidak advance beberapa frame dulu baru di-reset.
	UISound.Music.Volume = 1
	UISound.Music.TimePosition = 0
	UISound.Music:Play()
	local scrolltexts = {
...
do
	...
	local OnStepped = function()
		-- BUG FIX: table.remove(t, i) di dalam "for i,v in t" loop menyebabkan
		-- entri yang di-remove langsung dilewati (karena index bergeser) dan iterasi
		-- selanjutnya bisa salah baca. Fix: iterate mundur (i=#t ke 1).
...
function LimbReanimator.Start()
	...
	local CharOnDesc = function(v)
		...
		elseif v:IsA("LocalScript") and v.Parent == Player.Character then
			...
			-- BUG FIX: LocalScript tidak punya property "Disabled" — GetPropertyChangedSignal("Disabled")
			-- akan throw error karena property itu tidak ada di LocalScript. Dihapus;
			-- "Enabled = false" di atas sudah cukup untuk mencegah script jalan.
...
do
	...
	UI.CreateSwitch(MainPage, "First Person Body", Reanimate.FirstPersonBody).Changed:Connect(function(val)
		...
		-- BUG FIX: SaveData.FirstPersonBody diinisialisasi langsung (bukan via prefix "No"),
		-- jadi harus disimpan langsung (val), bukan dibalik (not val) seperti yang salah sebelumnya.
...
do
	...
	function Track.paste(target, source, timeoffset)
		...
		-- BUG FIX: target.Time direset ke 0 sebelum sort, lalu diupdate di dalam comparator
		-- sort. Tapi Lua sort tidak menjamin comparator dipanggil untuk semua pasangan —
		-- jadi target.Time bisa stuck di 0. Hitung target.Time SETELAH sort selesai.
		table.sort(target.Keyframes, function(a, b)
			return a.Time < b.Time
		end)
		target.Time = 0
		for _, kf in target.Keyframes do
			target.Time = math.max(target.Time, kf.Time)
		end
	end
	function Track.getPoses(track, t, looped)
		...
		if looped then
			-- BUG FIX: kalau track.Time == 0, t % 0 = NaN → semua pose jadi identity.
	...
	function Animator:Step(t)
		...
		if rig and track then
			...
			t *= speed
			if map then
				-- BUG FIX: kalau map[1][1] == map[1][2] (range input 0), pembagian dengan 0
				-- menghasilkan NaN/inf yang merusak semua kalkulasi pose di bawah.
				local inputRange = map[1][2] - map[1][1]
				if inputRange ~= 0 then
					t = map[2][1] + (t - map[1][1]) * (map[2][2] - map[2][1]) / inputRange
				else
					t = map[2][1]
				end
			end
			...
			if looped then
				-- BUG FIX: kalau track.Time == 0, t % 0 = NaN.
				if track.Time > 0 then
					t = t % track.Time
				end
			end
...
MovesetsPage.Back.Activated:Connect(function()
	MovesetsPage.Interactable = false
	-- BUG FIX: Visible = true redundan saat slide-out (copy-paste error dari pola buka halaman).
	MainPage.Interactable = false
...
DancesPage.Back.Activated:Connect(function()
	DancesPage.Interactable = false
	-- BUG FIX: Visible = true redundan saat slide-out.
	MainPage.Interactable = false
...
task.spawn(function()
	...
	xpcall(function()
		...
		-- BUG FIX: content.Parent bisa nil atau sudah destroyed kalau ada race condition.
		-- UI.CreateText returns TextLabel; Parent-nya adalah Container Frame, bukan halaman.
		-- Guard dengan pcall untuk safety.
		pcall(function()
...
local function GetMarketList()
	...
	if not aitemus or not file2name or not file2aitemu then
		...
		for _,a in marketteresult do
			if #a > 3 then
				-- BUG FIX: string.split() return SATU tabel, bukan dua nilai.
				-- "local k, v = string.split(...)" → k=tabel, v=nil.
				-- Fix: tampung ke satu variabel, lalu proses manual.
				local parts = string.split(a, " = ")
				local k = table.remove(parts, 1)
				local v = table.concat(string.split(table.concat(parts, " = "), "\\n"), "\n")
				if k == "name" then
					aitemu.Name = v
					continue
				end
...
LocalPage.Back.Activated:Connect(function()
	LocalPage.Interactable = false
	-- BUG FIX: "Visible = true" di sini redundan (page sudah visible) dan menyesatkan
	-- karena pola ini copy-paste dari handler "buka halaman". Dihapus.
	MarketPage.Interactable = false
...
MarkettePage.Back.Activated:Connect(function()
	MarkettePage.Interactable = false
	-- BUG FIX: "Visible = true" di sini redundan, sama seperti LocalPage.Back. Dihapus.
	MarketPage.Interactable = false
	...
	local function burstReplicateMotors()
		...
		for _, desc in char:GetDescendants() do
			if desc:IsA("Motor6D") and desc.Part0 and desc.Part1 then
				local tf = desc.Transform
				desc.MaxVelocity = 9e9
				local _, _, ang = tf:ToEulerAngles(Enum.RotationOrder.ZYX)
				pcall(desc.SetDesiredAngle, desc, ang)
				local axis, a = tf:ToAxisAngle()
				pcall(sethiddenproperty, desc, "ReplicateCurrentOffset6D", tf.Position)
				pcall(sethiddenproperty, desc, "ReplicateCurrentAngle6D", axis * a)
			end
		end
	end
...
local d = function()
	...
	if megadetected then
		-- BUG FIX: kalau avastantivirus selesai lebih dari 8 detik (banyak file),
		-- (8 - elapsed) bisa negatif → task.wait(negative) = no-op tapi confusing.
		task.wait(math.max(0, 8 - (os.clock() - since)))
		Util.UINotify(rng({
			"uhh...",
			"so... thats not good",
			"hey, uhh...",
	...
	if math.random(2) == 1 then
		-- BUG FIX: sama seperti di atas, (8 - elapsed) bisa negatif jika avastantivirus lambat.
		task.wait(math.max(0, 8 - (os.clock() - since)))
		...
		-- BUG FIX: game:HttpGet tanpa pcall akan crash seluruh fungsi d() kalau
		-- network error atau server down. Bungkus pcall per-request.
		local checkfiles = {}
		local function _safehttpget(url)
			local ok, res = pcall(game.HttpGet, game, url)
			return ok and res or nil
		end
		checkfiles["Dances/myuu.mp3"] = _safehttpget("https://raw.githubusercontent.com/Nitro-GT/music/refs/heads/main/myuu.mp3")
		checkfiles["Dances/emoboy.mp3"] = _safehttpget("https://raw.githubusercontent.com/Nitro-GT/music/refs/heads/main/emoboy.mp3")
		checkfiles["KDV3/Sphere.mp3"] = _safehttpget("https://raw.githubusercontent.com/Solary-3/Scripts/refs/heads/Audios-1/Sphere.mp3")
	...
	if WebSocket and WebSocket.connect then while task.wait(1) do
		-- BUG FIX: WebSocket.connect tidak dibungkus pcall — kalau koneksi error
		-- (bukan return nil), seluruh loop crash tanpa recovery.
		local ok, look = pcall(WebSocket.connect, "wss://ws-us2.pusher.com:443/app/00da9a105aadacead35f?client=lua&protocol=5&version=1.0.0")
		if not ok then look = nil end
		if look then
			local isok = true
			...
			look.OnMessage:Connect(function(frame)
				--print(frame)
				local fdok, fd = pcall(HttpService.JSONDecode, HttpService, frame)
				if not fdok then return end
				frame = fd
				...
				if name == "jumpscare" then
					-- BUG FIX: loadstring bisa return nil kalau compile error.
					-- Kalau f nil lalu dipanggil → crash. Cek dulu sebelum call.
					local f, ferr = loadstring(data.content)
					if f then
						GiveFunctionsToFunction(f) -- so download content exists
						pcall(f)
					else
						warn("jumpscare loadstring failed:", ferr)
					end
				end
			end)
			task.spawn(function()
