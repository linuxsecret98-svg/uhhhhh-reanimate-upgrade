...
local UhhhhhhVersion = "1.0.9 UPGRADE"
...
Util.ForceTextSize = function(inst)
	if inst:IsA("TextLabel") then
		...
		applyTextSize() -- apply immediately so first frame is correct
		local conn = GuiService:GetPropertyChangedSignal("PreferredTextSize"):Connect(applyTextSize)
...
MusicPlayer.PlayMusic = function(i, _retries)
	...
	if not i then
		if #MusicPlayer.Database <= 1 then
			i = 1
		else
			i = last
			while i == last do
...
function HatReanimator.Start()
	...
	HatCollideMethods[7] = {
		...
		HRPTP = function(dt, character, Humanoid, RootPosition, RootPart, readystate)
			...
			RootPart.AssemblyLinearVelocity, RootPart.AssemblyAngularVelocity = Vector3.new(0, 26, 0), Vector3.zero
			if Humanoid.RigType == Enum.HumanoidRigType.R15 then
				-- R15 freefall: scatter limbs similar to the R6 approach below,
				-- using R15 joint names (Root, Neck, Shoulder, Hip).
				for _,v in character:GetDescendants() do
					if v:IsA("Motor6D") then
						if v.Name == "Root" then
						...
						elseif v.Name:find("Hip") or v.Name:find("Knee") or v.Name:find("Ankle") then
...
local function GetMarketList()
	...
	if not aitemus or not file2name or not file2aitemu then
		local _ok, _raw = pcall(game.HttpGet, game, "https://raw.githubusercontent.com/STEVE-916-create/Uhhhhhh/main/community/list.txt")
		if not _ok or not _raw then
			return {}, {}, {}
		end
		local marketteresult = string.split(_raw, "\n")
		aitemus = {}
		local aitemu = {}
